...
Test`)
	...
	*/
	...
	r4 := strings.NewReader("Hello, Reader!")

	b3 := make([]byte, 8)
	for {
		n, err := r4.Read(b3)
		fmt.Printf("n = %v err = %v b3 = %v\n", n, err, b3)
		fmt.Printf("b3[:n] = %q\n", b3[:n])
		if err == io.EOF {
			break
		}
...
OuterLoop2:
	...
	// c6 := make(map[string]int)
	c6 := Counter{v: make(map[string]int)}
	go func() {
		for i := 0; i < 10; i++ {
			// c6["key"] += 1
			c6.Inc("Key")
